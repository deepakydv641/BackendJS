...
const tweetsApi = axios.create({
  baseURL: 'https://sharewithall.onrender.com/api/v1/tweets',
...
tweetsApi.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !original._retry) {
      ...
      try {
        const { data } = await axios.post('https://sharewithall.onrender.com/api/v1/users/refresh-access-token', {}, { withCredentials: true });
