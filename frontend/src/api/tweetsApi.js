...
const tweetsApi = axios.create({
  baseURL: (import.meta.env.MODE === 'development' ? 'http://localhost' : 'https://vidstream-th0g.onrender.com') + '/api/v1/tweets',
...
tweetsApi.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !original._retry) {
      ...
      try {
        const { data } = await axios.post(`${(import.meta.env.MODE === 'development' ? 'http://localhost' : 'https://vidstream-th0g.onrender.com')}/api/v1/users/refresh-access-token`, {}, { withCredentials: true });
